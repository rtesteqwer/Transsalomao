...
fs.copyFileSync(path.join(repo, 'render-overrides', '0012_report_daily_value.sql'), file('migrations/0012_report_daily_value.sql'));

{
  let s = read('src/lib/types.ts');
  if (!s.includes('dailyValue: number;')) {
    s = required(s, '  tons: number;\n  freightMode: FreightMode | null;', '  tons: number;\n  dailyValue: number;\n  freightMode: FreightMode | null;', 'DriverReport dailyValue');
  }
...

{
  ...
  if (!mapBlock.includes('dailyValue:')) {
    mapBlock = required(mapBlock, '    tons: num(r.tons),', '    tons: num(r.tons),\n    dailyValue: num(r.daily_value),', 'mapReport dailyValue');
    s = s.slice(0, mapStart) + mapBlock + s.slice(mapEnd + 3);
  }

  if (!s.includes('export const upsertDailyReport =')) {
    const marker = 'export const acceptReports =';
    const idx = s.indexOf(marker);
    if (idx < 0) throw new Error('daily-mode: acceptReports marker missing');
...

{
  ...
  if (!s.includes('    dailyReport,')) s = required(s, '    report,', '    dailyReport,\n    report,', 'dailyReport return');
...

{
  ...
  if (!s.includes('const [dailyValue, setDailyValue]')) {
    s = required(s, '  const [tons, setTons] = useState(\"\");\n  const [tripCount, setTripCount] = useState(\"1\");', '  const [tons, setTons] = useState(\"\");\n  const [dailyValue, setDailyValue] = useState(\"\");\n  const [tripCount, setTripCount] = useState(\"1\");', 'driver daily state');
  }
  s = required(s, '    const tonsN = parseLocaleNumber(tons);\n    const batchMode = freightMode === \"cegonha\" || freightMode === \"caixinha\";', '    const tonsN = parseLocaleNumber(tons);\n    const dailyValueN = parseLocaleNumber(dailyValue);\n    const batchMode = freightMode === \"cegonha\" || freightMode === \"caixinha\";', 'driver parse daily value');
  s = required(s, '    if (!freightMode) return toast.error(\"Escolha o modo de frete.\");\n    if (batchMode', '    if (!freightMode) return toast.error(\"Escolha o modo de frete.\");\n    if (freightMode === \"trip\" && (!dailyValueN || dailyValueN <= 0)) return toast.error(\"Informe o valor da diária.\");\n    if (batchMode', 'driver validate daily value');
  s = required(s, `        const res = await report.mutateAsync({\n          ticket: itemTicket,\n          driverId,\n          fleetId,\n          km: 0,\n          tons: batchMode ? 0 : (tonsN ?? 0),\n          freightMode,\n        });`, `        const res = freightMode === \"trip\"\n          ? await dailyReport.mutateAsync({\n              driverId,\n              fleetId,\n              tons: tonsN ?? 0,\n              dailyValue: dailyValueN ?? 0,\n            })\n          : await report.mutateAsync({\n              ticket: itemTicket,\n              driverId,\n              fleetId,\n              km: 0,\n              tons: batchMode ? 0 : (tonsN ?? 0),\n              freightMode,\n            });`, 'driver daily submit');
...

{
  ...
  s = s.replace('{r.freightMode === "cegonha" || r.freightMode === "caixinha" ? "Lançamento" : "Toneladas"}', '{r.freightMode === "trip" ? "Valor da diária" : r.freightMode === "cegonha" || r.freightMode === "caixinha" ? "Lançamento" : "Toneladas"}');
  s = s.replace('{r.freightMode === "cegonha" || r.freightMode === "caixinha" ? "1 viagem" : `${num(r.tons, 2)} t`}', '{r.freightMode === "trip" ? brl(r.dailyValue) : r.freightMode === "cegonha" || r.freightMode === "caixinha" ? "1 viagem" : `${num(r.tons, 2)} t`}');
  write('src/routes/dono/lancamentos.tsx', s);
}

for (const rel of ['src/lib/calc.ts','src/components/owner/trip-form.tsx','src/routes/motorista.tsx','src/routes/dono/lancamentos.tsx','src/routes/dono/viagens.tsx','src/routes/dono/totais.tsx','src/routes/dono/cadastros.tsx','src/routes/klebersom.tsx']) {
