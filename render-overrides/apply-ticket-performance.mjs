...
`;
  ...
  if (!s.includes('async function nextTicketCode')) {
    const nextTicketHelper = [
      ...
      '  const rows = await sql<{ next: number }>`select greatest(coalesce((select max(code::int) from trips where code ~ \'^[0-9]+$\'), 0), coalesce((select max(ticket::int) from reports where ticket ~ \'^[0-9]+$\'), 0)) + 1 as next`;',
