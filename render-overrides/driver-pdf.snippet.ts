async function generatePdf(data: any) {
  ...
  const drawHeader = () => {
    ...
    doc.text("Relatório operacional do motorista", 45, 13.5);
  ...
  autoTable(doc, {
    head: [["Data", "Ticket", "Conjunto", "Peso líquido", "Faturamento", "Comissão", "Após comissão"]],
    body: rows,
    startY: 25,
  ...
  for (let page = 1; page <= pages; page += 1) {
    ...
    doc.text(`Fretes: ${data.totals.trips}  •  Peso: ${exactTons(data.totals.totalTons)}  •  Faturamento: ${money(data.totals.billing)}`, 7, 205);
