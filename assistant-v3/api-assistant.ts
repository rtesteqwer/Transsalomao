...
async function localAnswer(message:string,history:Turn[]){
  ...
  if(text.includes("abastec")||text.includes("diesel")||text.includes("combustivel")){const r:any=await querySystem({operation:"fuelings",driver:d?.name||null,fleet:f?.name||null,date_from:null,date_to:null,limit:30});return{answer:`Encontrei ${r.count} abastecimento(s), totalizando ${brl(r.totalCost)}.`,data:r};}
  if(text.includes("despesa")||text.includes("gasto")){const r:any=await querySystem({operation:"expenses",driver:d?.name||null,fleet:f?.name||null,date_from:null,date_to:null,limit:30});return{answer:`Encontrei ${r.count} despesa(s), somando ${brl(r.total)}.`,data:r};}
  if(text.includes("pendente")||text.includes("caixa")||text.includes("lancamento")){const r:any=await querySystem({operation:"pending",driver:d?.name||null,fleet:f?.name||null,date_from:null,date_to:null,limit:50});return{answer:`Há ${r.count} lançamento(s) pendente(s).`,data:r};}
  ...
  if(text.includes("viagem")||text.includes("frete")){const r:any=await querySystem({operation:"trips",driver:d?.name||null,fleet:f?.name||null,date_from:null,date_to:null,limit:30});return{answer:`Encontrei ${r.count} viagem(ns), totalizando ${brl(r.totalFreight)} de frete.`,data:r};}
